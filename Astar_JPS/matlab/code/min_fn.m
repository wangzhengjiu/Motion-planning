...
%   Copyright 2009-2010 The MathWorks, Inc.
 ...
 for j=1:OPEN_COUNT
     if (OPEN(j,1)==1)
         ...
         if (OPEN(j,2)==xTarget && OPEN(j,3)==yTarget)
             flag=1;
             goal_index=j;%j表示在OPEN表的第几行 Store the index of the goal node
         end;
         k=k+1;
     end;
 ...
 if size(temp_array ~= 0)
  [min_fn,temp_min]=min(temp_array(:,8));%Index of the smallest node in temp array  %得到fn最小的点所在行
  i_min=temp_array(temp_min,9);%Index of the smallest node in the OPEN array   %得到fn最小的点在open表中的行
 else
     i_min=-1;%The temp_array is empty 此时open表也为空 i.e No more paths are available.
 end