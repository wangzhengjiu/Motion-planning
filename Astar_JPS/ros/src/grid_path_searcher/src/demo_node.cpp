...
Vector3d _start_pt;//列向量 3行1列 Eigen::Matrix<double,3,1> 表示一个列向量 3行1列
Vector3d _map_lower, _map_upper;         //界限
int _max_x_id, _max_y_id, _max_z_id;     //从0开始到最大
...
void rcvWaypointsCallback(const nav_msgs::Path & wp)
{     
    if( wp.poses[0].pose.position.z < 0.0 || _has_map == false ) //传
        return;
    //获取交互式界面给出的终点坐标
    Vector3d target_pt;
...
所需的所有信息和功能*/
 ...
 /*定义了结构体 JPSPathFinder 变量_jps_path_finder,该结构体存储、实现了JPS路径规划
所需的所有信息和功能*/
