...
for iter = 1:3000
    ...
    x_rand(1)=rand*xL;
    x_rand(2)=rand*yL;
    %plot(x_rand(1),x_rand(2),'*')
    x_near=[];
    %Step 2: 遍历树，从树中找到最近邻近点x_near 
    %提示：x_near已经在树T里
    T_point=cell2mat(struct2cell(T.v'));
    p_b=T_point(1:2,:);
    [~,ind]=min(dist(p_b',x_rand'));  %获取父节点到该采样点距离和父节点标号
    x_near=p_b(:,ind);
     
    %plot(x_near(1),x_near(2),'+')
    
    x_new=[];
    %Step 3: 扩展得到x_new节点
    %提示：注意使用扩展步长Delta
    vector=-x_near'+x_rand;   %求得x_near到x_rand的单位向量
    vector=vector/norm(vector);
    x_new=Delta*vector'+x_near;    %错误
     plot(x_new(1),x_new(2),'.')
    %检查节点是否是collision-free
    if ~collisionChecking(x_near,x_new,Imp) 
        continue;                         %结束本次循环进入到下次循环
    end
    count=count+1;
